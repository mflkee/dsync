...
def re_add_noctalia() -> GitResult:
    ...
    for f in noctalia_dir.iterdir():
        if not f.is_file():
            continue
        # chezmoi managed outputs target paths like .config/noctalia/settings.json
        target_path = str(f.relative_to(Path.home())).replace("\\", "/")
        # Also check source path format and tilde format
        source_path = "dot_config/noctalia/" + f.name
        tilde_path = "~/" + target_path
        if target_path in managed_files or source_path in managed_files or tilde_path in managed_files:
