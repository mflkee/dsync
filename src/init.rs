...
pub fn run() -> Result<()> {
    ...
    // 1. Роль машины.
    let roles = ["client (connects to a hub)", "hub + client (runs the hub AND syncs)", "hub only (server, no local projects)"];
    ...
    let cfg = Config {
        ...
        remote: if remotes.is_empty() { None } else { Some(remotes) },
    };
    ...
    std::fs::write(&target, format!(
        "# dsync config — generated by `dsync init`\n{rendered}"
    ))?;
...
fn projects_setup(
    ...
    loop {
        ...
        let machines = if remotes.is_empty() {
            ...
            println!(
                "{name}: machines to sync on? (space=select, enter=continue)"
            );
